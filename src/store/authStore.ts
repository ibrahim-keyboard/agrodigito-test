...
const mmkvStorage: StateStorage = {
  getItem: (key: string) => {
    ...
    logger.info(`MMKV getItem(${key}):`, value);
    return value ?? null;
  },
  setItem: (key: string, value: string) => {
    mmkv.set(key, value);
    logger.info(`MMKV setItem(${key}):`, value);
  },
  removeItem: (key: string) => {
    mmkv.delete(key);
    logger.info(`MMKV removeItem(${key})`);
  },
};

const logger = {
  info: (message: string, ...args: any[]) =>
    __DEV__ && console.log(`[AuthStore] ${message}`, ...args),
  error: (message: string, ...args: any[]) =>
    console.error(`[AuthStore] ${message}`, ...args),
};
...
interface AuthActions {
  ...
  checkProfileCompletion: () => Promise<
    'complete' | 'needs_name' | 'needs_shop_address'
  >;
    ...
    (set, get) => ({
      ...
      signInWithOTP: async (phone: string) => {
        ...
        if (!state.canResendOTP()) {
          const remainingTime = Math.ceil(
            (state.otpCooldown - Date.now()) / 1000
          );
      ...
      cleanup: () => {
        ...
        if (subscription) {
          ...
          logger.info('Auth subscription cleaned up');
        }
      },
    }),
    {
      ...
      onRehydrateStorage: () => (state, error) => {
        if (error) {
          logger.error('Rehydration error:', error);
          return;
        }
        logger.info('Auth state rehydrated:', state?.user?.email || 'No user');
      },
    }
  )
);
...
export function useAuth() {
  const { initialized, initialize, cleanup, user, session } = useAuthStore();

  useEffect(() => {
    if (!initialized) {
      logger.info('Initializing auth from useAuth hook...');
      initialize();
    }
